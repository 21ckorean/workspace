...
FROM EMP 
WHERE SAL > ANY (SELECT SAL FROM EMP WHERE DEPTNO = 30); #100, 200, 300
...
SELECT * FROM DEPT;
#3. '인천' 혹은 '서울' 지역에 근무하는 사원들의 성명, 직급을 조회( DEPT테이블)
SELECT ENAME, JOB
FROM EMP
WHERE DEPTNO IN (SELECT DEPTNO FROM DEPT WHERE LOC = '인천' OR LOC = '서울');

#4. 직급이 '과장'인 사원들이 받는 급여 모든 급여보다 적게 받는 직원들의 이름, 급여 조회
SELECT ENAME, SAL 
FROM EMP 
