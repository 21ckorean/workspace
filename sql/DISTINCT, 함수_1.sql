...
#DISTINCT 키워드는 하나만 붙이면 조회하는 모든 컬럼에 적용, 하나라도 다르면 중복아닌것으로 판단.
#모든 컬럼의 값이 일치하는 행을 중복으로 처리
SELECT JOB, DEPTNO FROM EMP;

#######################################################################

# 1. 수치 관련 함수 WHERE절 없으면 데이터 갯수만큼나옴
# CEIL : 올림, FLOOR : 버림, ROUND : 반올림
SELECT 70.5, CEIL(70.3), FLOOR(70.7), ROUND(70.5);

# TRUNCATE(숫자, 소수점자리) : 소수점자리까지 버림
...
SELECT SUBSTR('ABCDE', 3)
	, SUBSTR('ABCDE', 2, 3) #2번째 글자부터 추출, 3글자만
	, SUBSTRING('ABCDE', 3)
	, SUBSTRING('ABCDE', 2, 3);
...
SELECT TRIM(' DB '), LTRIM(' DB '), RTRIM(' DB ');
...
#7. 문자 대체 함수
SELECT REPLACE('나는 JAVA를 잘한다', 'JAVA', 'PYTHON');

#8. 논리함수
#8-1. IF(조건, 참일때값, 거짓일때값); 삼항연산자와 비슷함
SELECT IF(10 > 2, '참', '거짓');

#8-2. IFNULL(데이터, NULL일때 대체 데이터)
# ORACLE 디비에서는 IFNULL() 함수가 없음, 대신 NVL() 함수를 사용함. 기능은 동일
SELECT * FROM DEPT;
SELECT EMPNO, ENAME, COMM, IFNULL(COMM, 0), NVL(COMM, 0) FROM EMP;

#8-3. CASE 문법
#EX> 부서번호가 10 => 개발부, 부서번호가 20 => 영업부, 부서번호가 30 => 인사부
...
SELECT TITLE, AUTHOR FROM BOOK_STORE WHERE MOD(BOOK_ID, 2) = 1; # != 0; 도 홀수임.
#가격(PRICE)을 소수점 이하를 버리고(FLOOR) '정수가격'이라는 별칭으로 조회하세요.
SELECT FLOOR(PRICE) AS '정수가격' FROM BOOK_STORE; # '' 안써도됨.하지만 공백이있는별칭주려면 ''안에 써야함.

#[Part 2: 문자 함수]
#도서 제목(TITLE)의 좌우 공백을 제거하고, 모두 대문자로 변환하여 조회하세요.
SELECT TRIM(UPPER(TITLE)) FROM BOOK_STORE;
#저자(AUTHOR)의 이름이 3글자 이상이라면, 앞의 2글자만 추출하여 조회하세요.
#SELECT SUBSTR(AUTHOR, 1, 2) FROM BOOK_STORE WHERE CHAR_LENGTH(AUTHOR) >= 3; 내가 한것
SELECT IF(CHAR_LENGTH(AUTHOR) >= 3, SUBSTR(AUTHOR, 1, 2), AUTHOR) FROM BOOK_STORE; #정답
...
SELECT RPAD(AUTHOR, 5, '*') FROM BOOK_STORE; #LPAD나 RPAD를 쓰는데 글자수가 정해놓은 수보다 많으면 짤려서 조회됨.
...
SELECT *, IF(STOCK = 0, '품절', '판매중') FROM BOOK_STORE; #전체 컬럼옆에 하나 추가하려면 *, 추가하고싶은컬럼 하면됨.
...
SELECT DISTINCT CATEGORY
	, CASE CATEGORY
		WHEN CATEGORY = 'IT' THEN '1층 로비'
		WHEN CATEGORY = 'DB' THEN '2층 사무실'
		WHEN CATEGORY = 'NOVEL' THEN '3층 서고'
		ELSE '기타'
		END AS '위치'
	FROM BOOK_STORE;
