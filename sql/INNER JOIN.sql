...
#테이블에 별칭 줄 때는 AS 키워드 붙이지 마세요! (오라클에선 붙이면 오류남.)
SELECT E.EMPNO, E.ENAME, E.SAL
FROM EMP E; #왜 가능할까? SELECT보다 FROM이 먼저 해석되기 때문!
...
# 사번, 사원명, 급여, 부서명, 지역 조회
# 두 테이블에 공통으로 존재하는 컬럼 조회 시 주의 필요!!
SELECT EMPNO, ENAME, SAL, DNAME, LOC, EMP.DEPTNO, DEPT.DEPTNO
FROM EMP, DEPT;

# Inner JOIN
# 조인조건 : 조인하고자 하는 두 테이블이 공통으로 가진 컬럼이 같은 값만 조회한다는 조건 추가
