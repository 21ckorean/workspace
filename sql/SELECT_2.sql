...
#EMP테이블에서 급여가 400이거나, 급여가 600이거나 800인 사원들의
#모든 정보를 조회
SELECT * 
FROM emp
...
FROM books
WHERE STOCK = 'NULL' OR STOCK = 0;

#저자의 성이 **'이'**씨인 도서들을 조회하여 제목 기준 오름차순으로 정렬하시오. (별칭을 활용하여 제목은 '도서명', 저자는 '글쓴이'로 표시)
SELECT TITLE AS '도서명', AUTHOR AS '글쓴이'
FROM books
WHERE AUTHOR LIKE'이__'
ORDER BY TITLE;
...
SELECT *
FROM books
WHERE (CATEGORY = 'IT' AND PRICE >= 30000) OR (CATEGORY = '소설' AND PRICE <= 15000);
...
#저자가 **'이데이터'**이거나 **'김철수'**인 도서 중, 2021년 이후에 출판된 도서의 제목, 저자, 출판일을 조회하시오. 
#(단, 출판일 기준 내림차순 정렬)
...
FROM books
WHERE PRICE BETWEEN 15000 AND 30000 AND STOCK IS NOT NULL
