...
SELECT COMM, IFNULL(COMM, 10) FROM EMP; #단일행함수의 예시, IFNULL()결과가 NULL이면 10으로 해주는 함수
...
#데이터의 개수 및 평균을 구할 때는 NULL데이터는 제외하여 계산함!
...
#GROUP BY - 특정 기준에 따라 그룹을 묶어 집계 데이터를 조회할 때 사용, 집계함수랑 같이 사용
#모든 사원의 급여 평균 조회
SELECT AVG(SAL) FROM EMP;

#직급별 사원의 급여 평균 조회
#GROUP BY 절을 사용하면 SELECT 절에 그룹핑 기준이 되는 컬럼은 사용 가능!!
SELECT JOB, AVG(SAL)
FROM EMP
GROUP BY JOB;
...
FROM EMP
WHERE DEPTNO != 30 #WHERE절이 GROUP BY 절보다 앞에와야함.
GROUP BY JOB
...
#Q1. 사번이 짝수인 사원들에 대해 부서별로 급여의 합 및 평균을 조회한다.
#	단, 부서별 급여의 합이 300인 데이터만 조회하고,
#	부서번호 기준 내림차순으로 정렬
...
#SELECT 쿼리문의 해석 순서(컴퓨터가 해석 순서)*********시험 중요
