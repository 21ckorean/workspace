...
# 강혜정 사원보다 급여를 더 만이 받는 사원들의 사번, 사원명, 급여를 조회
...
# 김사랑 사원의 사번, 사원명, 직급, 부서번호, 부서명을 조회(join, sub query)
SELECT EMPNO
	, ENAME
	, JOB
	, DEPTNO
...
SELECT CART_NUM
		...
		, (SELECT BOOK_TITLE
		   FROM SHOP_BOOK 
		   WHERE BOOK_NUM = SHOP_CART.BOOK_NUM) AS TITLE
		, (SELECT BOOK_PRICE 
		   FROM SHOP_BOOK 
		   WHERE BOOK_NUM = SHOP_CART.BOOK_NUM) AS PRICE
FROM SHOP_CART;
...
/*SELECT MEM_EMAIL
FROM SHOP_CART 
WHERE (SELECT COUNT(CART_NUM)
	   FROM SHOP_CART) >= 3;*/

#회원 이메일이 'aa'인 회원이 장바구니에 담은 상품 종류의 수
