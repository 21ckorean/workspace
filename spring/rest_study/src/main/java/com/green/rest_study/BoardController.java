...
public class BoardController {
  ...
  //게시글 하나를 조회하는 API
  //URL -> (GET) localhost:8080/boards/5  <<5번 조회

  //URL에 {}를 사용하여 변수처럼 활용할 수 있음
  //이 변수는 URL에 적용되었다는 의미에서 URL Parameter라 부름
  //url parameter로 전달되는 데이터는 메서드의 매개변수에 @PathVariable 어노테이션의 사용으로 활용 가능
  ...
  @GetMapping("/{age}/{name}") //age라는 url주소에 5를 넣고 그 5를 변수 num에 넣겠습니다.
  ...
  //게시글 삭제 API
  //URL -> (DELETE) localhost:8080/boards <<<전체삭제
  //URL -> (DELETE) localhost:8080/boards/3 <<<3번만 삭제
  ...
  //게시글 수정 API
  //URL -> (PUT) localhost:8080/boards/2 <<<2번만 수정
